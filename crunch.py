...
class record:
  def __init__(self, date, month, day, year, dead, k):
    self.date = date
    self.month = month
    self.day = day
    self.year = year
    ...
    self.k = k
  def __str__(self):
    return "{0},{1},{2}".format(self.date,self.dead,self.k)
...
def monthly_numbers(results, month, country):
  ...
  for day in range(start_day, end_day):
    ...
    r = record('{0}-{1}-2020'.format(zero_pad(month), zero_pad(day)), month, day, 2020, dead, k) 
    results.append(r)
